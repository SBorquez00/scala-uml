...
export default function ContextMenu({ id, top, left, right, bottom, ...props }: ContextMenuProps) {
  ...
  return (
    <div style={{ top, left, right, bottom }} className="bg-white border shadow absolute z-10 h-fit w-fit" {...props}>
      <p style={{ margin: '0.5em' }} className='text-xl'>
        <small>node: {id}</small>
      </p>
      <button onClick={deleteNode} className='border-t-2 border-black block p-2 text-left w-full text-2xl hover:bg-stone-300'>Borrar</button>
    </div>
  );
}
